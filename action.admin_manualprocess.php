...
// Redirect to https?
if ($this->GetPreference('force_ssl','0') == 1 && (! isset($_SERVER['HTTPS']) || empty($_SERVER['HTTPS']) || $_SERVER['HTTPS'] == 'off')) {
...
if ( !isset($_SERVER['HTTPS']) || empty($_SERVER['HTTPS']) || $_SERVER['HTTPS'] == 'off') $warnings[] = $this->Lang('warning_https');
if( $order_obj->get_status() == ORDERSTATUS_PAID ) {
    $warnings[] = $this->Lang('warning_orderstatus_paid');
}
else if( $order_obj->get_status() == ORDERSTATUS_CANCELLED ) {
    $warnings[] = $this->Lang('warning_orderstatus_cancelled');
}
if( $ccprocessing_gateway == -1 )  $warnings[] = $this->Lang('warning_no_ccprocessing_gateway');
...
if( isset($params['payment_id']) ) {
    ...
    if( isset($params['process_only']) ) $payment->set_method(\Orders\Payment::TYPE_CREDITCARD);
}
else {
...
if( isset($params['cancel']) ) {
    unset($params['cancel']);
    $this->CGRedirect($id,'admin_manageorder',$returnid,$params);
}
else if (isset($params['submit']) ) {
    //
    // rebuild the payment object from params.
    //
    $tmp = mktime($params['payment_date_Hour'], $params['payment_date_Minute'], 0,
                  $params['payment_date_Month'], $params['payment_date_Day'], $params['payment_date_Year']);
    ...
    if( $payment->get_cc_number() == '' && $payment->get_method() == \Orders\Payment::TYPE_CREDITCARD ) {
        $errors[] = $this->Lang('error_invalidfield',$this->Lang('creditcard_number'));
    }
    if( $payment->get_cc_verifycode() == '' && $payment->get_method() == \Orders\Payment::TYPE_CREDITCARD ) {
        $errors[] = $this->Lang('error_invalidfield',$this->Lang('creditcard_verifycode'));
    }
    if( $payment->get_method() == \Orders\Payment::TYPE_CREDITCARD && $payment->get_cc_expiry() < time() ) {
    ...
    if( !count($errors) ) {
        // save this payment.
        $res = $payment->save();
        if( !$res ) $errors[] = $this->Lang('error_save','payment');
    }

    if( $payment->get_method() == \Orders\Payment::TYPE_CREDITCARD &&
        isset($params['process_cc_new']) &&
        $params['process_cc_new'] == 1 ) {
        ...
        if( !count($errors) && !$gateway_module ) $errors[] = $this->Lang('error_nopaymentgateway');
        if( !count($errors) && !$gateway_module->RequiresCreditCardInfo() ) $errors[] = $this->Lang('error_invalidgateway');
        ...
        if( !count($errors) ) {
            ...
            foreach( $order_obj->get_destinations() as $shipment ) {
                foreach( $shipment->get_items() as $item ) {
                    $gateway_module->AddItem($item->get_description(), $item->get_item_id(),
                                             $item->get_quantity(),$item->get_weight(),
                                             $item->get_net_price());
        ...
        $this->CGRedirect($id,'admin_manageorder',$returnid,$parms);
    }
}
    ...
    $datakey = '';
    if( isset($_GET['cntnt01datakey']) ) $datakey = $_GET['cntnt01datakey'];

    $gateway_module->RestoreState($datakey);
    if( $gateway_module->GetOrderId() != $orderid )	$errors[] = $this->Lang('error_gateway_invalid_data');
    ...
    if( !count($errors) ) {
	    ...
	    {
            // update the order
            $payment->set_status(\Orders\Payment::STATUS_APPROVED);
...
//
if( count($warnings) ) $smarty->assign('warnings',$warnings);
if( count($errors) ) $smarty->assign('errors',$errors);

$statuses = array();
$statuses[\Orders\Payment::STATUS_APPROVED] = $this->Lang(\Orders\Payment::STATUS_APPROVED);
$statuses[\Orders\Payment::STATUS_DECLINED] = $this->Lang(\Orders\Payment::STATUS_DECLINED);
$statuses[\Orders\Payment::STATUS_ERROR] = $this->Lang(\Orders\Payment::STATUS_ERROR);
$statuses[\Orders\Payment::STATUS_CANCELLED] = $this->Lang(\Orders\Payment::STATUS_CANCELLED);
$statuses[\Orders\Payment::STATUS_OTHER] = $this->Lang(\Orders\Payment::STATUS_OTHER);
$statuses[\Orders\Payment::STATUS_PENDING] = $this->Lang(\Orders\Payment::STATUS_PENDING);
$statuses[\Orders\Payment::STATUS_NOTPROCESSED] = $this->Lang(\Orders\Payment::STATUS_NOTPROCESSED);
$smarty->assign('statuses',$statuses);

$pmt_types = array();
$pmt_types[\Orders\Payment::TYPE_ONLINE] = $this->Lang(\Orders\Payment::TYPE_ONLINE);
$pmt_types[\Orders\Payment::TYPE_CASH] = $this->Lang(\Orders\Payment::TYPE_CASH);
$pmt_types[\Orders\Payment::TYPE_CREDITCARD] = $this->Lang(\Orders\Payment::TYPE_CREDITCARD);
$pmt_types[\Orders\Payment::TYPE_UNKNOWN] = $this->Lang(\Orders\Payment::TYPE_UNKNOWN);
$smarty->assign('pmt_types',$pmt_types);

if( isset($params['process_only']) ) $smarty->assign('process_only',1);
$smarty->assign('formstart',$this->CGCreateFormStart($id,'admin_manualprocess',$returnid,$params));
$smarty->assign('url',cge_url::current_url());
$smarty->assign('formend',$this->CreateFormEnd());
$smarty->assign('order_obj',$order_obj);
$tmp = $payment->get_assocdata();
if( is_array($tmp) && count($tmp) ) $smarty->assign('payment_assoc',$tmp);
