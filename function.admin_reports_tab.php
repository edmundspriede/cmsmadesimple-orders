...
$smarty->assign('formstart',
		$this->CGCreateFormStart($id,'admin_report',$returnid,
					 array('showtemplate'=>'false'),
					 FALSE,'post','','', 'target="blank"'));
...
for( $i = 0; $i < count($files); $i++ ) {
    ...
    $classname = '\\Orders\\reports\\'.$tmp[1];
    if( is_array($tmp) && count($tmp) == 3 ) $report_opts[$classname] = $this->Lang($tmp[1]);
}
