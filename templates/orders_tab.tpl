...
<a href="#" id="filter_btn" class="pointer">{cgimage image='icons/system/view.gif' alt=''} {$mod->Lang('filter')}</a>
...
{if !isset($orders) || count($orders) == 0}
  <div class="red" style="width: 98%; text-align: center;">{$Orders->Lang('records_matching_query')}: {$recordcount}</div>
{else}
<div class="pageoverflow">
    ...
    <tr>
      ...
      <td>
        {if $order.status == 'paid'}
          <span style="color: green;">{$Orders->Lang($order.status)}</span>
	{else}
	   {$Orders->Lang($order.status)}
	{/if}
      </td>
      <td>{$order.items}</td>
