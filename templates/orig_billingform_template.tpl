...
{if isset($order)}
  ...
  <div class="billing_info">
    <fieldset>
      ...
      <div class="row">
        <p class="col-sm-4 text-right required">{$Orders->Lang('postal')}:</p>
        <p class="col-sm-8">
      ...
      <div class="row">
        <p class="col-sm-4 text-right required">{$Orders->Lang('phone')}:</p>
        <p class="col-sm-8">
          <input type="text" name="{$actionid}billing_phone" value="{$billing->get_phone()}" size="20"/>
        </p>
      </div>
      <div class="row">
        <p class="col-sm-4 text-right required">{$Orders->Lang('fax')}:</p>
        <p class="col-sm-8">
          <input type="text" name="{$actionid}billing_fax" value="{$billing->get_fax()}" size="20"/>
        </p>
      </div>
      <div class="row">
        <p class="col-sm-4 text-right required">{$Orders->Lang('email')}:</p>
        <p class="col-sm-8">
          <input type="text" name="{$actionid}billing_email" value="{$billing->get_email()}" size="40"/>
        </p>
      </div>
    </fieldset>
  </div>
  ...
  *}
  {for $i = 0 to $num_destinations - 1}
    ...
    <div class="shipping_info">
      <fieldset>
        <legend>{if $num_destinations == 1}{$Orders->Lang('shipping_info_if_different')}:{else}{$Orders->Lang('shipping_info_for')}: {$dest->get_name()}{/if}
	...
	{if $dest->allows_pickup()}
        <div class="row">
           ...
           <select class="col-sm-8" id="do_pickup" name="{$actionid}shipping_{$i}_pickup">
              {cge_yesno_options selected="{$dest->get_pickup()}"}
           </select>
        </div>
        {/if}
        ...
        <div class="row">
          <p class="col-sm-4 text-right required">{$Orders->Lang('email')}:</p>
          <p class="col-sm-8">
            <input type="text" name="{$actionid}shipping_{$i}_email" value="{$addr->get_email()}" size="40"/>
          </p>
        </div>
        <div class="row">
  ...
  <fieldset>
    ...
    <div class="row">
      ...
      <p class="col-md-8">
         <select name="{$actionid}orders_extra_fld1">{cge_yesno_options selected=$order->get_extra('fld1')}</select>
      </p>
    </div>
    <div class="row">
  ...
  <fieldset>
    ...
    <div class="alert alert-info">{$Orders->Lang('info_order_notes')}</div>
    <div class="row">
      <p class="col-sm-4 text-right required">{$Orders->Lang('dest_notes')}:</p>
      <p class="col-sm-8">
