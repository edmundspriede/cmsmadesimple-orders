...
$(function() {
  ...
  $('#print_packinglist').click(function(ev){
     ...
     var url = '{cms_action_url module=Orders action=admin_print_packinglist orderid=$order->get_id() shipping_id=XXX forjs=1}&showtemplate=false';
  ...
  jQuery("[name$='submit']").click(function() {
	...
	var res = confirm('{$Orders->Lang('ask_setorderstatus')}'+want_status);
	if( res ) {
...
<h3>{$Orders->Lang('title_manage_order')}</h3>
<fieldset>
<legend>{$Orders->Lang('order_statistics')}: </legend>
<div class="c_full cf">
  <div class="grid_6">
    <label class="grid_2 text-right">{$Orders->Lang('order_number')}:</label>
    <span class="grid_10">{$order->get_invoice()}</span>
    <label class="grid_2 text-right">{$Orders->Lang('status')}:</label>
    {if !empty($order_statuses)}
      ...
      <input class="cms_submit" type="submit" name="{$actionid}submit" value="{$Orders->Lang('submit')} "/>
      </div>
    {else}
      <span>{$Orders->Lang($order->get_status())}</span>
    {/if}

    {if $can_manage_orders}
    <div class="clearb"></div>
    <div class="c_full cf" style="margin-top: 0.5em;">
      <a class="grid_6" href="javascript:void()" onclick="openwindow('{$print_url}'); return false;">{cgimage image='printer.png'} {$Orders->Lang('print_invoice')}</a>
      <a class="grid_6" href="{cms_action_url module=Orders action=admin_createinvoice orderid=$order->get_id()}">{cgimage image='report.png'} {$Orders->Lang('create_invoice')}</a>
    </div>
    {/if}
  </div>{* column 1 *}

  <div class="grid_5" style="border-left: 1px solid gray;">
    <label class="grid_3 text-right">{$Orders->Lang('username')}:</label>
    {capture assign='username'}{$Orders->GetUsername($order->get_feu_user())}{/capture}
    {if $username == ''}
      <span class="grid_9" style="color: red;">{$Orders->Lang('unknown')}</span>
    {else}
      <span class="grid_9">{$username}</span>
    {/if}

    <label class="grid_3 text-right">{$Orders->Lang('created')}:</label>
    <span class="grid_9">{$order->get_create_date()|cms_date_format}</span>

    <label class="grid_3 text-right">{$Orders->Lang('last_modified')}:</label>
    <span class="grid_9">{$order->get_modified_date()|cms_date_format}</span>

    <label class="grid_3 text-right">{$Orders->Lang('subtotal')}:</label>
    <span class="grid_9">{$currencysymbol}{$order->get_subtotal()|number_format:2}</span>

    <label class="grid_3 text-right">{$Orders->Lang('discount')}:</label>
    <span class="grid_9">{$currencysymbol}{$order->get_discount()|number_format:2}</span>

    <label class="grid_3 text-right">{$Orders->Lang('shipping_total')}:</label>
    <span class="grid_9">{$currencysymbol}{$order->get_shipping_cost()|number_format:2}</span>

    <label class="grid_3 text-right">{$Orders->Lang('tax')}:</label>
    <span class="grid_9">{$currencysymbol}{$order->get_tax_cost()|number_format:2}</span>

    <label class="grid_3 text-right">{$Orders->Lang('total')}:</label>
...
<fieldset class="c_full cf">
    {capture assign='tmp'}<em>({$Orders->Lang('edit')})</em>{/capture}
    <legend>{$Orders->Lang('bill_to')}: {if isset($can_manage_orders)}{module_action_link module='Orders' action='admin_editorderbilling' text=$tmp orderid=$order->get_id()}{/if}</legend>
    ...
    <span class="grid_12">{$Orders->Lang('phone')}:&nbsp;{$billing->get_phone()}</span>
    <span class="grid_12">{$Orders->Lang('fax')}:&nbsp;{$billing->get_fax()}</span>
    <span class="grid_12">{$Orders->Lang('email_address')}:&nbsp;{$billing->get_email()}</span>
...
<fieldset>
  <legend>{$Orders->Lang('payment_info')}:</legend>
  ...
  {if $can_manage_orders}
  <a class="grid_12" href="{cms_action_url module=Orders action=admin_manualprocess orderid=$order->get_id()}">{cgimage image='money_add.png'} {$Orders->Lang('process_manual_payment')}</a>
  {/if}
  ...
  {if $order->count_payments()}
    ...
    <table class="pagetable" cellspacing="0">
	 ...
	 <td>{$Orders->Lang($payment->get_status())}</td>
         <td>
	   {$Orders->Lang($payment->get_method())}
	   {if $payment->get_gateway()}/ {$payment->get_gateway()}{/if}
         ...
         <td>
          {if $can_manage_orders && $payment->get_status() == 'payment_notprocessed' && $payment->get_method() == 'creditcard'}
            {module_action_link module='Orders' action='admin_manualprocess' imageonly=1 image='creditcard.png' text=$Orders->Lang('prompt_process_cc_now') orderid=$order->get_id() payment_id=$payment->get_id() process_only=1}
          {/if}
         </td>
         <td>
          {if $can_manage_orders}
            {module_action_link module='Orders' action='admin_manualprocess' imageonly=1 image='icons/system/edit.gif' text=$Orders->Lang('edit') orderid=$order->get_id() payment_id=$payment->get_id()}
          {/if}
         </td>
         <td>
          {if $can_manage_orders}
            {module_action_link module='Orders' action='admin_deletepayment' imageonly=1 image='icons/system/delete.gif' text=$Orders->Lang('delete') orderid=$order->get_id() payment_id=$payment->get_id() confmessage=$Orders->Lang('ask_delete_payment')}
          {/if}
         </td>
       </tr>
     {/foreach}
...
{function display_shipping}
  ...
  <div class="c_full cf">
	...
	<div class="grid_12">{$Orders->Lang('phone')}: {$addr->get_postal()}</div>
        {if $addr->get_fax() != ''}<div class="grid_12">{$Orders->Lang('fax')}: {$addr->get_fax()}</div>{/if}
        {if $addr->get_email() != ''}<div class="grid_12">{$Orders->Lang('email_address')}: {$addr->get_email()}</div>{/if}
     </div>
     <div class="grid_5">
       {$packing_list=$dest->get_packing_list()}
       {if $packing_list}
          <a class="grid_12" href="#" id="print_packinglist" data-shipping-id="{$dest->get_id()}">{cgimage image='printer.png'} {$mod->Lang('print_packing_list')}</a>
       {/if}
       {if $dest->get_message() != ''}
       <fieldset>
         <legend>{$Orders->Lang('shipping_message')}:</legend>
     ...
     {if $dest->count_items() }
     <fieldset>
       <legend>{$Orders->Lang('items')}: </legend>
       <table class="pagetable" cellspacing="0">
         <thead>
           <tr>
             <th>{$Orders->Lang('source')}</th>
             <th>{$Orders->Lang('id')}</th>
             <th>{$Orders->Lang('name')}</th>
             <th>{$Orders->Lang('type')}</th>
             <th>{$Orders->Lang('subscription')}</th>
         ...
         {foreach from=$dest->get_items() item='item'}
	      ...
	      <td>{if $item->is_subscription()}{$Orders->Lang('yes')}{else}{$Orders->Lang('no')}{/if}</td>
...
<fieldset>
  <legend>{$Orders->Lang('destinations')}:</legend>
...
<div class="pageoverflow">
  <p class="pageinput">
   <input type="submit" name="{$actionid}submit" id="submit" value="{$Orders->Lang('submit')}"/>
   {$cancel}
  </p>
</div>
{$formend}
