...
//
$max_destinations = -1;
$thetemplate = cge_param::get_string($params,'template',$this->GetPreference('dflt_billingform_template'));
$tpl = $this->CreateSmartyTemplate($thetemplate,'billingform_');
if( !\cg_ecomm::is_config_ok() ) {
    $this->ShowFormattedMessage('CGEcommerceBase is not completely/properly configured... cannot continue',TRUE,'CRITICAL ERROR');
    return;
}
...
try {
    ...
    if( $uid === FALSE ) throw new exception($this->Lang('error_notloggedin'));
    ...
    if( $order_id ) {
        ...
        else if( $tmp->get_status() != ORDERSTATUS_PROPOSED ) {
    ...
    if( isset($params['invoice_prefix']) ) \Orders\orders_helper::set_invoice_prefix($params['invoice_prefix']);

    $policy = $this->GetPreference('address_retrieval',\Orders\billing_address_retriever::ADDR_POLICY_NONE);
    ...
    if( !$tmp->is_valid() ) {
        ...
        if( $tmp_addr && $tmp_addr->is_valid() )  $order_maker->set_billing_address( $tmp_addr );
    }
    if( ! $order_maker->supports_different_shipping_locations() ) $max_destinations = 1;
    ...
    if ($uid !== FALSE) {
        if (isset($params['submit'])) {
            ...
            if( isset($params['billing_firstname']) ) {
                $billing_addr = new \Orders\Address();
            ...
            if( isset($params['order_notes']) ) {
                $notes = cge_utils::clean_input_html(cms_html_entity_decode($params['order_notes']));
            ...
            for( $i = 0; $i < 1000; $i++ ) {
                if( !isset($params['shipping_'.$i.'_firstname']) ) break;

                $addr = new \Orders\Address();
                $addr->from_array($params,'shipping_'.$i.'_');
                $order_maker->set_shipping_address($i,$addr);
                $order_maker->set_shipping_pickup($i,\cge_param::get_bool($params,"shipping_{$i}_pickup"));
                $order_maker->set_shipping_message($i,trim(strip_tags(cge_utils::get_param($params,"shipping_{$i}_message"))));
            }
            ...
            if( empty($status) ) {
                ...
                //
                session_write_close();
                audit($order->get_id(),'Orders','Customer proceeding to confirm page');
...
if( $max_destinations > 0 && $num_destinations > 0 ) $num_destinations = min($num_destinations,$max_destinations);
...
$tpl->assign('formstart',$this->CGCreateFormStart($id, 'default', $returnid, $params));
$tpl->assign('formend',$this->CreateFormEnd());

$promotions_module = cge_utils::get_module('Promotions');
if( isset($promotions_module) ) $tpl->assign('promotions_avail',1);
$tpl->display();
