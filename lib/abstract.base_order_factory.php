...
abstract class base_order_factory implements order_factory
{
    ...
    public function set_cart_module(\CGEcommerceBase\shopping_cart_mgr $module)
    {
        if( $module->SupportsMultipleBaskets() && $this->supports_multiple_vendors() ) {
            throw new \RuntimeException('Sorry, the cart module selected cannot be used in this configuration as the order maker can support orders from multiple vendors.');
        }
    ...
    {
        $new_cart_item = new \cg_ecomm_cartitem($line_item->get_sku(),$line_item->get_item_id(),$line_item->get_quantity(),$line_item->get_source());
        ...
        if( $line_item->get_master_price() > 0 ) $new_cart_item->set_item_total($line_item->get_master_price());
    ...
    protected function to_line_item(\cg_ecomm_cartitem $basket_item)
    {
        $new_line_item = null;
        ...
        if( $t_promo > 0 ) $new_line_item->set_extra('promotion_id',$t_promo);
    ...
    protected function find_promotion_items(array $cart_items)
    {
        ...
        // this should prolly belong in the Promotions module or CGEcommerceBase
        ...
        if( !is_array($offers) || !count($offers) ) return;
        ...
        foreach( $offers as $offer ) {
            ...
            if( is_object($offer) ) {
                ...
                case $offer::OFFER_PRODUCTID:
                    ...
                    if( $dims )  $free_cart_item->set_dimensions( $dims[0], $dims[1], $dims[2] );
                    break;
                    ...
                    if( ($idx = $offer->get_cart_idx()) > -1 ) {
                        ...
                        if( $idx >= 0 && $idx < count($cart_items) ) {
                            ...
                            if( ($item->get_unit_discount() == '' && $item->get_parent() == -1) ||
                                ...
                                $val = min($val,1.0);
                                $discount = round($item->get_unit_discount() + $item->get_unit_price()*$val*-1,2);
            ...
            if( $free_cart_item ) $out[] = $free_cart_item;
        }
    ...
    private function _calculate_item_taxes(\CGEcommerceBase\tax_calculator $taxmodule, LineItem $oneitem,&$cumulative_taxes)
    {
    ...
    protected function setup_tax_module(Destination $dest)
    {
        ...
        if( !$taxmodule || !$taxmodule->IsConfigured() ) return;

        // Provide billing information to the taxes module
        // for this order maker, the source address is the cgecommerce company address
        $taxmodule->set_source_location( \cg_ecomm::get_company_address() );
    ...
    protected function calculate_taxes(Destination $dest, $items)
    {
        ...
        if( is_array($items) && count($items) ) {
            foreach( $items as $oneitem ) {
                ...
                case LineItem::ITEMTYPE_SHIPPING:
                    if( \cg_ecomm::can_tax_shipping() ) $this->_calculate_item_taxes($taxmodule, $oneitem, $results);
                    break;

                case LineItem::ITEMTYPE_PRODUCT:
                    $source = $oneitem->get_source();
                    $product_info = \cg_ecomm::get_product_info( $source, $oneitem->get_item_id() );
                    $taxable = $product_info->get_taxable();
                    if( $taxable ) $this->_calculate_item_taxes($taxmodule, $oneitem, $results);
                    break;
        ...
        if( !is_array($results) || count($results) == 0 ) return;
        $line_items = array();
