...
final class orders_ops
{
    ...
    static public function find_last_feu_order($feu_uid)
    {
        ...
        $query = 'SELECT id FROM '.cms_db_prefix().'module_orders WHERE feu_user_id = ? ORDER by id DESC LIMIT 1';
    ...
    {
        $db = cmsms()->GetDb();
        $query = 'SELECT id FROM '.cms_db_prefix().'module_orders WHERE billing_email = ? ORDER BY id DESC';
        $tmp = $db->GetCol($query,array(trim($email)));
        if( is_array($tmp) && count($tmp) ) return $tmp;
    }

    static public function find_by_feu_id($uid)
    {
        $db = cmsms()->GetDb();
        $query = 'SELECT id FROM '.cms_db_prefix().'module_orders WHERE feu_user_id = ? ORDER by id DESC';
    ...
    static public function &load_by_invoice($invoice)
    {
        ...
        $query = 'SELECT id, feu_user_id, invoice, billing_company, billing_first_name, billing_last_name, billing_address1,
              ...
              FROM '.cms_db_prefix().'module_orders WHERE invoice = ?';
        ...
        $query = 'SELECT id FROM '.cms_db_prefix().'module_orders_shipping WHERE order_id = ?';
        ...
        $query = 'SELECT id FROM '.cms_db_prefix().'module_orders_payments WHERE order_id = ?';
        $tmp = $db->GetCol($query,array($id));
        if( is_array($tmp) ) {
        ...
        $query = 'SELECT id FROM '.cms_db_prefix().'module_orders_messages WHERE order_id = ? ORDER BY sent ASC';
        $tmp = $db->GetCol($query,array($id));
        if( is_array($tmp) ) {
    ...
    static public function insert(Order &$order)
    {
        ...
        $query = 'INSERT INTO '.cms_db_prefix()."module_orders (feu_user_id,invoice, billing_company,billing_first_name,billing_last_name,
        ...
        $extra = null;
        if( ($tmp = $order->get_all_extra()) ) $extra = serialize($tmp);

        $billing = $order->get_billing();
        $dbr = $db->Execute($query,
                            array($order->get_feu_user(),'NOTSET',
                                  $billing->get_company(),
    ...
    static public function delete_by_id($order_id)
    {
        ...
        $assoc = new \CGExtensions\AssocData($db,'Orders');
