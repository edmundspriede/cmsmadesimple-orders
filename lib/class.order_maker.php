...
class order_maker extends base_order_factory
{
    ...
    public function __construct(Order $order = null)
    {
        ...
        $this->_this_module = \cms_utils::get_module(MOD_ORDERS);
        $this->_cart_module = \cg_ecomm::get_cart_module();
        $this->set_shipping_policy(\cg_ecomm::get_system_shipping_policy());
    }
  ...
  protected function _adjust_line_items_for_discounts($uid,&$items)
  {
      ...
      \CMSMS\HookManager::do_hook('Orders::CalculateDiscounts',$params);
  ...
  public function set_shipping_policy(\CGEcommerceBase\shipping_policy $policy)
  {
  ...
  public function supports_different_shipping_locations() { return TRUE; }

  public function validate_addresses()
  {
      $errors = null;
      if( !$this->_order->get_billing()->is_valid() ) $errors[] = $this->_this_module->Lang('err_invalid_billing_address');

      $policy = \cg_ecomm::get_system_shipping_policy();
  ...
  protected function get_raw_lineitems(Destination $destination)
  {
      ...
      foreach( $line_items as $one ) {
          if( $one->get_extra('__meta__') ) continue;
          $out[] = $one;
      }
      return $out;
  }
  ...
  public function adjust_for_shipping()
  {
      ...
      if( $this->_handling_module && $this->_handling_module->IsConfigured() ) {
          ...
          if( is_array($tmp) && count($tmp) == 2 && (float)$tmp[0] > 0 ) {
              ...
              $desc = $tmp[1];
              if( !$tmp ) $desc = $this->Lang('handling');
      ...
      for( $d = 0; $d < $numd; $d++ ) {
          ...
          if( $this->_packaging_module ) {
              ...
              $packing_list = new \CGEcommerceBase\packing_list( $destination->get_shipping_address(), $cart_items );
          }
          if( $packing_list ) $destination->set_packing_list( $packing_list );
          ...
          if( $packing_list && $this->_shipping_module && !$destination->get_pickup() ) {
              ...
              else if( $shipping_cost instanceof \CGEcommerceBase\shipping_estimate ) {
                  $line_item = new LineItem();
  ...
  public function get_basic_order()
  {
      ...
      if( $this->_cart_module->GetNumItems() == 0 ) throw new \CmsException($this->_this_module->Lang("no_items_in_cart"));
      if( ! $this->_shipping_policy ) throw new \RuntimeException($this->_this_module->Lang('err_noshippingpolicy'));
      ...
      if( !$billing_addr ) {
          $billing_addr = $order->get_billing();
          if( !$billing_addr ) $billing_addr = new \cge_address;
      }
      ...
      if( $this->_cart_module ) {
          ...
          foreach( $basket_names as $one_basket_name ) {
              ...
              //$shipping_addr = new \Orders\Address();
              ...
              if( $shipping_addr ) $destination->set_shipping_address($shipping_addr);

              // set the source where stuff is coming from for this destination/shipping/sub-order
              $destination->set_source_address( \cg_ecomm::get_company_address() );

              if( isset($basket_details['dest_message']) ) {
                  $msg = \cge_utils::clean_input_html($basket_details['dest_message']);
                  $destination->set_message($msg);
              }

              $line_items = $this->_get_line_items($destination);
              if( !$line_items ) continue;
              ...
              if( $added ) $order->add_shipping($destination);
          }
      }
