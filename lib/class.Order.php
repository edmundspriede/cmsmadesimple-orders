...
final class Order
{
    ...
    {
        $this->_id = $val;
        if( $set_invoice )$this->set_invoice(orders_helper::make_invoice_number($this->get_id()));
    }
    ...
    {
        if( $reset_id ) $shipping->set_id(null);
        $shipping->set_order_id($this->get_id());
        if( !is_array($this->_shipping_recs) ) $this->_shipping_recs = array();
    ...
    public function del_shipping($idx)
    {
        ...
        for( $i = 0; $i < count($this->_shipping_recs); $i++ ) {
            if( $i != $idx ) $new_data[] = $this->_shipping_recs[$i];
        }
    ...
    {
        for( $i = 0; $i < count($this->_shipping_recs); $i++ ) {
            if( $this->_shipping_recs[$i]->get_name() == $name ) return $this->_shipping_recs[$i];
        }
        $out = null;
        return $out;
    }
    ...
    {
        for( $i = 0; $i < count($this->_shipping_recs); $i++ ) {
            if( $this->_shipping_recs[$i]->get_id() == $id ) return $this->_shipping_recs[$i];
        }
        $out = null;
        return $out;
    }
    ...
    {
        if( $idx >= 0 && $idx < count($this->_shipping_recs) ) return $this->_shipping_recs[$idx];
        $out = null;
        return $out;
    }
    ...
    {
        if( $reset_id ) $payment->set_id(null);
        $payment->set_order_id($this->get_id());
        if( !is_array($this->_payment_recs) ) $this->_payment_recs = array();
    ...
    {
        if( $idx >= 0 && $idx < count($this->_payment_recs) ) return $this->_payment_recs[$idx];
    }
    ...
    public function &get_payment_by_id($pmt_id)
    {
        ...
        for( $i = 0; $i < count($this->_payment_recs); $i++ ) {
            if( $this->_payment_recs[$i]->get_id() == $pmt_id ) $res =& $this->_payment_recs[$i];
        }
        return $res;
    }
    ...
    public function &get_payment_by_txn_id($txn_id)
    {
        $res = null;
        for( $i = 0; $i < count($this->_payment_recs); $i++ ) {
            if( $this->_payment_recs[$i]->get_txn_id() == $txn_id ) $res =& $this->_payment_recs[$i];
        }
        return $res;
    }
    ...
    {
        if( $reset_id ) $message->set_id(null);
        $message->set_order_id($this->get_id());
        if( !is_array($this->_note_recs) ) $this->_note_recs = array();
    ...
    {
        if( $idx >= 0 && $idx < count($this->_note_recs) ) return $this->_note_recs[$idx];
    }

    public function get_note_by_id($note_id)
    {
        for( $i = 0; $i < count($this->_note_recs); $i++ ) {
            if( $this->_note_recs[$i]->get_id() == $note_id ) return $this->_note_recs[$i];
        }
    }
    ...
    public function get_amount_paid()
    {
        ...
        for( $i = 0; $i < $this->count_payments(); $i++ ) {
            ...
            if( $payment->get_status() != Payment::STATUS_APPROVED ) continue;
    ...
    {
        if( $this->get_id() > 0 ) return orders_ops::update($this,$del_children);
    ...
    {
        if( !is_array($this->_extra) ) return $dflt;
        if( isset($this->_extra[$key]) ) return $this->_extra[$key];
        return $dflt;
    }
    ...
    public function set_extra($key,$value)
    {
        $key = trim($key);
        if( !$key ) return;
        if( !is_array($this->_extra) ) $this->_extra = array();
        $this->_extra[$key] = $value;
    }

    public function isset_extra($key)
    {
        if( !is_array($this->_extra) ) return FALSE;
        return isset($this->_extra[$key]);
    }

    public function unset_extra($key)
    {
        if( $this->isset_extra($key) ) unset($this->_exttra[$key]);
    }
    ...
    public function from_array($data,$prefix='billing_')
    {
        ...
        if( isset($data['extra']) ) {
            if( is_string($data['extra']) ) $data['extra'] = unserialize($data['extra']);
            if( count($data['extra']) ) {
    ...
    public function to_array()
    {
        ...
        if( is_array($this->_extra) && count($this->_extra) ) $result['extra'] = $this->_extra;
