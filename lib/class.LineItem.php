...
class LineItem
{
    ...
    private $_subscr_payperiod = -1;  // subscription info.
    ...
    {
        return max($this->get_master_price(), $this->get_unit_price()*$this->get_quantity())
            + $this->get_unit_discount()*$this->get_quantity();
    }

    public function is_subscription()
    {
        if( empty($this->_subscr_payperiod) || $this->_subscr_payperiod == '-1' || $this->_subscr_payperiod == 'none' ) return FALSE;
        return TRUE;
    }
    ...
    {
        if( !is_array($this->_assocdata) ) $this->_assocdata = array();
    ...
    {
        if( $this->get_id() > 0 ) return lineitem_ops::update($this);
    ...
    public function from_array($data)
    {
        ...
        if( isset($data['description']) ) $this->set_description($data['description']);
