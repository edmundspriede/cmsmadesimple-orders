...
final class gateway_helper
{
    ...
    // if any.
    static public function get_compatible_gateways(Order $order_obj)
    {
        $tmp_gateway_modules = \cg_ecomm::get_gateway_modules();
        if( !is_array($tmp_gateway_modules) || !count($tmp_gateway_modules) ) return;
        ...
        foreach( $tmp_gateway_modules as $mod_name ) {
            ...
            if( !is_object($mod) ) continue; // module in list, but could not be loaded.

            $policy = $mod->get_cartitem_policy();
            if( !is_object($policy) ) continue; // no policy (technically an error)
        ...
        if( count($valid_names) ) return $valid_names;
    }

    static public function process_gateway_transaction(\CGEcommerceBase\async_transaction $trans)
    {
        $gCms = cmsms();
        $db = $gCms->GetDb();
        ...
        if( !$order_id ) return $orders_mod->Lang('error_orderinvalid');
        ...
        if( $uid && $order_obj->get_feu_user() != $uid ) return $orders_mod->Lang('error_orderinvalid');

        $billing = $order_obj->get_billing();
        $email_addr = $billing->get_email();
        if( !$email_addr ) return $orders_mod->Lang('error_noemailaddress');
        ...
        if( !$payment ) {
            ...
            if( $payment_id > 0 ) $payment = $order_obj->get_payment_by_id($payment_id);
        }
        if( !$payment ) {
        ...
        case \CGEcommerceBase\payment_gateway::PAYMENT_STATUS_AUTHORIZED:
            $do_orderstatus = FALSE;
            $payment->set_status(Payment::STATUS_AUTHORIZED);
            $orders_mod->Audit($order_id,$orders_mod->GetName(),sprintf("%.2f authorized for order %d (transaction id: %s)",$amount,$order_id,$transaction_id));
            return;
        ...
        case \CGEcommerceBase\payment_gateway::PAYMENT_STATUS_PENDING:
            ...
            if( $cur_status == ORDERSTATUS_PROPOSED  || $cur_status == ORDERSTATUS_PENDING || $cur_status == ORDERSTATUS_CONFIRMED) {
        ...
        if( $do_orderstatus ) {
            ...
            case ORDERSTATUS_INCOMPLETE:
                if( $status == \CGEcommerceBase\payment_gateway::PAYMENT_STATUS_CANCELLED ) {
                    $order_obj->set_status(ORDERSTATUS_CANCELLED);
                }
                else if( $status == \CGEcommerceBase\payment_gateway::PAYMENT_STATUS_PENDING ) {
            ...
            case ORDERSTATUS_SUBSCRIBED:
                ...
                case \CGEcommerceBase\payment_gateway::PAYMENT_STATUS_DECLINED:
                    // ADD A NOTE
                    $order_obj->set_status(ORDERSTATUS_HOLD);
        ...
        if( !$sendemail ) return; // we're done.
        ...
        if( $status == \CGEcommerceBase\payment_gateway::PAYMENT_STATUS_APPROVED ) {
