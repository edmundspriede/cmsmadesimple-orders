...
final class shipping_ops
{
    ...
    {
        $id = (int) $id;
        if( $id < 1 ) throw new \LogicException('Invalid destination id passed to '.__METHOD__);
        // load the shipping record itself
        $gCms = \CmsApp::get_instance();
        $db = $gCms->GetDb();
        $query = 'SELECT * FROM '.cms_db_prefix().'module_orders_shipping WHERE id = ?';
        ...
        // load all the items.
        $query = 'SELECT id FROM '.cms_db_prefix().'module_orders_items WHERE shipping_id = ? ORDER BY item_type ASC';
  ...
  static public function insert(Destination &$shipping)
  {
      ...
      $query = 'INSERT INTO '.cms_db_prefix()."module_orders_shipping (order_id,name,
      ...
      if( !$src_address ) $src_address = new \Orders\Address; /// ugly hack
      $dest_address = $shipping->get_shipping_address();
      if( !$dest_address ) $dest_address = new \Orders\Address; /// ugly hack
  ...
  static public function delete_by_id($id)
  {
      ...
      // a. delete the items
      $query = 'DELETE FROM '.cms_db_prefix().'module_orders_items WHERE shipping_id = ?';
      $dbr = $db->Execute($query,array($id));
      if( !$dbr ) return FALSE;

      // b. delete the shipping.
      $query = 'DELETE FROM '.cms_db_prefix().'module_orders_shipping WHERE id = ?';
      $dbr = $db->Execute($query,array($id));
      if( !$dbr ) return FALSE;
  ...
  static public function load_bulk( array $idlist )
  {
      ...
      $sql = 'SELECT * FROM '.cms_db_prefix().'module_orders_shipping WHERE id IN ('.implode(',',$idlist).') ORDER BY id';
      $dest_recs = $db->GetArray($sql);

      // get the line items
      $sql = 'SELECT * FROM '.cms_db_prefix().'module_orders_items WHERE shipping_id IN ('.implode(',',$idlist).') ORDER BY shipping_id,id';
      ...
      foreach( $dest_recs as $dest_rec ) {
          ...
          for( $i = 0, $n = count($line_recs); $i < $n; $i++ ) {
              $rec = $line_recs[$i];
              if( $rec['shipping_id'] < $dest_rec['id'] ) continue;
              if( $rec['shipping_id'] > $dest_rec['id'] ) break;
              $obj = new LineItem();
              $obj->from_array( $rec );
              $list[] = $obj;
          }
