...
class Destination
{
    ...
    const STATUS_COMPLETED   = '_completed'; // no further action necessary (digital orders)
    ...
    private $_optional;
    private $_vendor_id;
    private $_pickup; // set for local pickup.
    private $_status;

    // these members are not saved to the database.
    private $_allow_pickup; // used for order selection only.
    ...
    public function set_packing_list(\CGEcommerceBase\packing_list $obj)
    {
    ...
    public function set_shipping_address(\cge_address $val)
    {
    ...
    public function set_source_address(\cge_address $val)
    {
    ...
    {
        if( !$this->_status ) return self::STATUS_PENDING;
        return $this->_status;
    }
    ...
    public function get_extra($key)
    {
        ...
        if( is_array($this->_optional) && isset($this->_optional[$key]) ) return $this->_optional[$key];
    }
    ...
    public function set_extra($key,$val)
    {
        $key = trim($key);
        if( !$key ) throw new \LogicException("Invalid key passed to ".__METHOD__);
        if( !is_array($this->_optional) ) $this->_optional = [];
    ...
    {
        if( !is_array($items) || count($items) == 0 ) return; // @todo throw an exception

        $this->_lineitems = array();
        foreach( $items as $item ) {
            if( !is_object($item) ) continue; // @todo throw an exception
            if( ! $item instanceof LineItem ) continue; // @todo throw an exception
            $item->set_id(null);
            $item->set_order_id($this->get_order_id());
            $item->set_shipping_id($this->get_id());
            $this->_lineitems[] = $item;
    ...
    {
        if( !is_array($skus) && is_string($skus) ) $skus = array($skus);
        for( $i = 0; $i < count($this->_lineitems); $i++ ) {
            if( in_array($this->_lineitems[$i]->get_sku(),$skus) ) return $this->_lineitems[$i];
        }
        $out = null;
        return $out;
    }
    ...
    {
        if( $this->get_status() == self::STATUS_SHIPPED || $this->get_status() == self::STATUS_DELIVERED || $this->get_status() == self::STATUS_PICKUPREADY ) {
            return TRUE;
        }
        return FALSE;
        /*
        for( $i = 0; $i < $this->count_all_items(); $i++ ) {
            $item =& $this->_lineitems[$i];
            if( $item->get_status() != ITEMSTATUS_SHIPPED && $item->get_item_type() == LineItem::ITEMTYPE_PRODUCT ) return false;
        }
        return TRUE;
        */
    }
    ...
    public function count_services()
    {
        ...
        for( $i = 0; $i < $this->count_all_items(); $i++ ) {
            $item =& $this->_lineitems[$i];
            if( $item->get_item_type() == LineItem::ITEMTYPE_SERVICE ) $n_s++;
        }
        return $n_s;
    }
    ...
    public function is_subscription_only()
    {
        ...
        for( $i = 0; $i < $this->count_all_items(); $i++ ) {
            ...
            else if( $item->get_item_type() == LineItem::ITEMTYPE_PRODUCT ||
                     $item->get_item_type() == LineItem::ITEMTYPE_SERVICE ) {
    ...
    public function count_products()
    {
        ...
        for( $i = 0; $i < $this->count_all_items(); $i++ ) {
            $item =& $this->_lineitems[$i];
            if( $item->get_item_type() == LineItem::ITEMTYPE_PRODUCT ) $n_p++;
        }
        return $n_p;
    }
    ...
    public function from_array($data)
    {
        ...
        if( !empty($data['option']) ) $this->set_optional(unserialize($data['option']));
        if( !empty($data['packing_list']) ) {
            // packing list can be null.
            $tmp = unserialize($data['packing_list']);
            if( $tmp ) $this->set_packing_list( $tmp );
        }
    ...
    public function to_array()
    {
        ...
        if( $obj ) $result['packing_list'] = serialize($obj);
