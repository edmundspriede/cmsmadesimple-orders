...
class Payment
{
  ...
  {
    $this->_payment_date = \cge_utils::unix_time($val);
  }
  ...
  {
    return cge_string::mask_string($this->get_cc_number,-4,$mask);
  }
  ...
  public function get_extra($key)
  {
      ...
      if( is_array($this->_assocdata) ) {
          if( isset($this->_assocdata[$key]) ) $ret = $this->_assocdata[$key];
      }
  ...
  {
    if( $this->get_id() > 0 ) return payment_ops::update($this);
  ...
  public function from_array($data)
  {
    ...
    if( isset($data['order_id']) ) $this->set_order_id($data['order_id']);
    ...
    if( isset($data['assocdata']) && is_array($data['assocdata']) )	$this->_assocdata = $data['assocdata'];
  }
