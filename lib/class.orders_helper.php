...
class orders_helper extends \module_helper
{
    ...
    {
        $module = self::instance();
        if( $module->CheckPermission(ORDERS_PERM_MANAGEORDERS) ) return TRUE;
        return FALSE;
    }
    ...
    static public function make_invoice_number($order_id,$prefix = '')
    {
        if( !$prefix ) {
            ...
            if( !is_null(self::$_invoice_prefix) ) $prefix = self::$_invoice_prefix;
        }
    ...
    {
        return \cg_ecomm_utils::is_creditcard_number($str);
    }

    static public function requires_creditcard_number()
    {
        stack_trace(); die('__fixme__');
        $gateway_module = \cg_ecomm::get_payment_module();
        if( is_object($gateway_module) ) return $gateway_module->RequiresCreditCardInfo();
        return FALSE;
    }
    ...
    static public function creditcard_requires_ssl()
    {
        ...
        if( is_object($gateway_module) ) $gateway_requires_ssl = $gateway_module->RequiresSSL();
    ...
    static public function is_valid_user()
    {
        ...
        if( !in_array( $grpid, $gids ) ) return FALSE;
    ...
    public static function estimate_packages(Order& $order)
    {
        ...
        foreach( $dests as $one_dest ) {
            ...
            foreach( $items as $one_item ) {
                ...
                if( $one_item->get_item_type() != LineItem::ITEMTYPE_PRODUCT ) continue;
                ...
                if( !is_object($info) ) throw new Exception('Could not get product info for a product');
                ...
                if( !\cge_units::is_weight_metric(\cg_ecomm::get_weight_units()) ) $box['weight'] *= cge_units::KG_TO_LBS;
        ...
        if( !$res ) return $generator->get_error(); // error ooccurred
    ...
    {
        $badresult = NULL;
        if( $uid <= 0 ) $uid = self::is_valid_user();
        ...
        if( !$feu && !$module->GetPreference('allow_anon_checkout',0) ) return $badresult;
        ...
        foreach( $addr_map as $key => $value ) {
            ...
            if( $value == -1 || $value == '' ) $new_addr_array[$key] = '';
    ...
    public static function get_gateway_confirm_form($order,$gwmod,$returnid,$destpage = 0)
    {
        ...
        if( ! $order instanceof Order ) throw new \CmsInvalidDataException('Expecting an order object in parameter 1 of '.__METHOD__);
        if( ! $gwmod instanceof \CGPaymentGatewayBase ) throw new \CmsInvalidDataException('Expecting a payment gateway module in parameter 2 of '.__METHOD__);
        if( $returnid < 1 ) $returnid = \cms_utils::get_current_pageid();
        if( $destpage < 1 ) {
