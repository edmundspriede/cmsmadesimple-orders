...
#-------------------------------------------------------------------------
if( !isset($gCms) ) exit;
if( !$this->GetPreference('allow_manual_checkout',0) ) exit;
...
// Connect with FrontEndUsers
    ...
    $thetemplate = \cge_param::get_string($params,'invoicetemplate',$this->GetPreference('dflt_invoice_template'));
    ...
    $smarty->assign('orders_formstart',$this->CGCreateFormStart($id,'complete',
                                                            $returnid, array('order_id'=>$order_id)));
    ...
    // Update the order
    //
    $order_obj->set_status(ORDERSTATUS_INVOICED);
    $order_obj->save();

    //
    // Process the template
    //
    $rsrc = $this->CGGetTemplateResource($thetemplate,'invoice_');
    ...
    //
    audit($order_obj->get_id(),'Orders','Customer viewed the invoice (cart cleared)');
}
catch( \Exception $e ) {
